...
describe 'Notifications Worker process' do
  ...
  describe 'once_a_month' do
    ...
    it 'should run only once in a month' do
      now = Time.now
      expect(queue_manager).to receive(:enqueue).exactly(1)
  ...
  describe 'frequency' do
    before do
      create :notification, :max_read_difference, frequency: 'daily'
      create :notification, :max_read_difference, frequency: 'weekly'
      create :notification, :max_read_difference, frequency: 'monthly'
    end
